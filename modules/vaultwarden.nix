...
, ...
  ...
  config = lib.mkIf config.secshell.vaultwarden.enable {
    sops = {
      ...
      templates."vaultwarden/env".content = let
      ...
      in ''
        ...
        ${lib.optionalString (config.secshell.vaultwarden.smtp.hostname != null && config.secshell.vaultwarden.smtp.from != null) "SMTP_USERNAME=${config.sops.placeholder."vaultwarden/smtpUsername"}"}
        ${lib.optionalString (config.secshell.vaultwarden.smtp.hostname != null && config.secshell.vaultwarden.smtp.from != null) "SMTP_PASSWORD=${config.sops.placeholder."vaultwarden/smtpPassword"}"}
      '';
    };
